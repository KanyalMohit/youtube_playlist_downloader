...
ipcMain.on('download-audio', (event, { url, format, isPlaylist }) => {
  ...
  const outputTemplate = path.join(selectedDownloadFolder, isPlaylist ? '%(playlist)s/%(title)s.%(ext)s' : '%(title)s.%(ext)s').replace(/\\/g, '/');
  ...
  // Track when a song is actually completed
  ...
  download.on('close', (code) => {
    if (code === 0) {
      ...
      event.sender.send('download-error', `Download failed with exit code ${code}`);
    }
  });
